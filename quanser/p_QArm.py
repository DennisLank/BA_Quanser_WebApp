...
class QArm():
    ...
    def __init__(self, hardware=1):
        ...
        self.hardware = hardware # Only Hardware support is enabled.
        self.card = HIL()
        if self.hardware:
            board_identifier = "0"
        else:
...
class QArmUtilities():
    ...
    def qarm_forward_kinematics(self, phi):
        ...
        #ACHTUNG DAVOR Falsch--> "theta = phi" -- ist eine Refernz keine Kopie! ACHTUNG
        theta = np.copy(phi)
        theta[0] = phi[0]
        ...
        p0   = np.array([0, 0, 0])       # Roboterbasis im Ursprung
        p1   = T01[0:3, 3]               # Position nach erstem Gelenk
        p2   = T02[0:3, 3]               # Position nach zweitem Gelenk
        p3   = T03[0:3, 3]               # Position nach drittem Gelenk
        p4   = T04[0:3, 3]               # Orientierung des Endeffektors
